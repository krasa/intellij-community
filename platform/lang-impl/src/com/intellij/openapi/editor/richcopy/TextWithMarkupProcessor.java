  ...
  public TextBlockTransferableData collectTransferableData(@Nullable PsiFile file, Editor editor, int[] startOffsets, int[] endOffsets) {
    ...
    try {
      ...
      for (int i = 0; i < startOffsets.length; i++) {
        ...
        final DisposableIterator[] disposableIterators;
        if (file != null) {
          MarkupModel documentMarkupModel = DocumentMarkupModel.forDocument(document, file.getProject(), false);
          disposableIterators = new DisposableIterator[]{wrap(highlighter, text, schemeToUse, startOffsetToUse, endOffsetToUse),
            wrap(documentMarkupModel, text, schemeToUse, startOffsetToUse, endOffsetToUse),
            wrap(editor.getMarkupModel(), text, schemeToUse, startOffsetToUse, endOffsetToUse)};
        }
        else {
          disposableIterators = new DisposableIterator[]{wrap(highlighter, text, schemeToUse, startOffsetToUse, endOffsetToUse),
            wrap(editor.getMarkupModel(), text, schemeToUse, startOffsetToUse, endOffsetToUse)};
        }
        DisposableIterator<SegmentInfo> it = aggregateSyntaxInfo(schemeToUse, disposableIterators);
        try {
          while (it.hasNext()) {
